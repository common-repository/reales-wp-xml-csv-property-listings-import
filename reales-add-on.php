...
$reales_addon->import_images( 'reales_addon_photo_gallery', 'Photo Gallery' );

function reales_addon_photo_gallery( $post_id, $attachment_id, $image_filepath, $import_options ) {
...
$reales_addon->import_files( 'reales_addon_property_plans', 'Property Plans' );

function reales_addon_property_plans( $post_id, $attachment_id, $image_filepath, $import_options ) {
...
$reales_addon_city_setting = get_option( 'reales_prop_fields_settings' );

if ( !empty( $reales_addon_city_setting['reales_p_city_t_field'] ) && $reales_addon_city_setting['reales_p_city_t_field'] == 'list' ) {

	// 'list' = custom list. 'google' = google autocomplete.

	$reales_addon->add_field( 'property_city', 'City', 'text', null, 'The City will be added to your custom list if it does not exist.' );
...
$reales_addon->admin_notice(
	...
	array( 
		'themes' => array( 'Reales WP', 'RealesWP' )
) );
...
function reales_addon_import( $post_id, $data, $import_options, $article ) {
    ...
    foreach ( $fields as $field ) {
        ...
        if ( empty( $article['ID'] ) or $reales_addon->can_update_meta( $field, $import_options ) ) {
    ...
    if ( empty( $article['ID'] ) or $reales_addon->can_update_image( $import_options ) ) {
    ...
    foreach ($fields as $field) {
		...
		// Hat tip to /libs/migrate_amenities.php
		
		$underscored = str_replace(' ', '_', $field);
		$sanitized = reales_substr45(sanitize_title($underscored));
		$field_name = sanitize_key($sanitized);

    	if ( empty( $article['ID'] ) or $reales_addon->can_update_meta( $field_name, $import_options ) ) {
    ...
    if (  empty( $article['ID'] ) or ( $reales_addon->can_update_meta( $field, $import_options ) && $reales_addon->can_update_meta( 'property_location', $import_options ) && !empty ( $search ) ) ) {
    ...
    if ( ( empty( $reales_addon_city_setting['reales_p_city_t_field'] ) or !isset( $reales_addon_city_setting['reales_p_city_t_field'] ) or $reales_addon_city_setting['reales_p_city_t_field'] == 'google' ) ) {

    	$fields['property_city'] = $city;

    } else {

    	$cities = get_option( 'reales_cities_settings' );

    	if ( !empty( $cities ) ) {

			foreach( $cities as $key => $value ) {
    			
    			if( strtolower( $value['name'] ) == strtolower( $data['property_city'] ) ) {
    			
    				$found = true;
    			
    				if ( empty( $article['ID'] ) or $reales_addon->can_update_meta( 'property_city', $import_options ) ) {
    			
    					update_post_meta( $post_id, 'property_city', $key );
    		...
    		if ( empty( $found ) ) {
    			...
    			if ( isset( $data['property_city'] ) && !empty( $data['property_city'] ) ) {
	    			...
	    			$city_id = sanitize_title_with_dashes( $data['property_city'] );
	    			
	    			$city_id = str_replace( '-', '_', $city_id );

	    			$city_position = end( $cities );

	    			$city_position = apply_filters( 'wpai_reales_city_position', $city_position['position'] + 1 );

	    			$cities[$city_id] = array(
	    				'id' => $data['property_city'],
	    				'name' => $data['property_city'],
	    				'position' => $city_position
	    				);

	    			update_option( 'reales_cities_settings', $cities );

	    			if ( empty( $article['ID'] ) or $reales_addon->can_update_meta( 'property_city', $import_options ) ) {

	    				update_post_meta( $post_id, 'property_city', $city_id );
    ...
    foreach ( $fields as $key => $value ) {
        ...
        if ( empty( $article['ID'] ) or $reales_addon->can_update_meta( $key, $import_options ) ) {
	...
	if ( $geocoding_failed ) {
		$reales_addon->log( "WARNING Geocoding failed with status: " . $details['status'] );
		if ( array_key_exists( 'error_message', $details ) ) {
			$reales_addon->log( "WARNING Geocoding error message: " . $details['error_message'] );
		}
	}
}
